...
class Airflow_Test_Manager (assisipy_utils.darc.manager.DARC_Manager):
    ...
    def monitor (self):
        ...
        for a in self.atm_config ['arenas']:
            ...
            self.atm_config['parameters'][0] = airflow_test_4_worker.START_CORE
            zmq_sock_utils.send (a ['socket_core'], self.atm_config ['parameters'])
            self.atm_config['parameters'][0] = airflow_test_4_worker.START_LEAF
...
def create_DARC_config_file (list_casus):
    contents = {
        'controllers': {
            'domset': {
                'main': check_file (airflow_test_4_worker.__file__),
                'extra': [
