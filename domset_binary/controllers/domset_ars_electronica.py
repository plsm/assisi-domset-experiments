...
_AIRFLOW_TEMPERATURE = 28
"""
Temperature used when simulating airflow
"""
...
class DomsetController(Thread):
    ...
    def update(self):
        ...
        if not (self.temp_ref_old == self.temp_ref) and self.blow == 0.0:
    ...
    def airflow_control(self):
        if self.blow > 0.0:
            if (self.blow_prev == 0.0):
                ...
                self.casu.set_temp (_AIRFLOW_TEMPERATURE)
                if (self.casu_id == 20):
                ...
                if (time_now - self.start_blow) > self.blow:
                    ...
                    self.temp_ref = self.casu.get_temp (assisipy.casu.TEMP_WAX)
                    if (self.casu_id == 20):
